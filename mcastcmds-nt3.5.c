...
size_t size=lseek(fdin,0L,SEEK_END);
...
/*note: offset has to be multi.of 2^12*/
   fgetcn(fdin1,buffer,i,BUF_SIZ);
  
readnf(fdin1,filebuffer,i,BUF_SIZ);
mycpy(buffer,filebuffer,BUF_SIZ);
mmapf(fdin,buffer,i,BUF_SIZ);
/*
...
fgetcn(fdin1,buffer,i,rem);
	readnf(fdin1,filebuffer,i,rem);
	mycpy(buffer,filebuffer,rem);
mmapf(fdin,buffer,i,rem);
 /*
buffer[MCASTBUF_SIZ-3]=srcflag;
n=0; d=0;
while((n=sendto(sock,buffer+d,MCASTBUF_SIZ-d, 0, (struct sockaddr *) &mcast, sizeof(mcast)))>0){
